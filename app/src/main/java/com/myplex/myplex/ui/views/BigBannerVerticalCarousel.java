...
import com.myplex.myplex.ui.adapter.AdapterBigBannerVerticalCarousel;
...
public class BigBannerVerticalCarousel extends GenericListViewCompoment {

    private List<CarouselInfoData> mListCarouselInfo;
    private Context mContext;
    private static final String TAG = BigBannerVerticalCarousel.class.getSimpleName();
    private RecyclerView mRecyclerViewCarouselInfo;
    private View view;
    private final VerticalSpaceItemDecoration mHorizontalMoviesDivieder;
    private EventSearchMovieDataOnOTTApp searchMovieData;
    private String mMenuGroup;
    private String mPageTitle;

    AdapterBigBannerVerticalCarousel adapterBigBannerVerticalCarousel;

    String mBgColor;
    GenericListViewCompoment holder;
    private Typeface mBoldTypeFace;
    public BigBannerVerticalCarousel(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo,
                                     String mMenuGroup, String mPageTitle,String mBgColor) {
        super(view);
        this.view = view;
        ...
        mHorizontalMoviesDivieder = new VerticalSpaceItemDecoration(margin);
    ...
    public static BigBannerVerticalCarousel createView(Context context, ViewGroup parent,
        ...
        View view = LayoutInflater.from(context).inflate(R.layout.listitem_carousel_vertical_recycler, parent, false);
        return new BigBannerVerticalCarousel(context, view, carouselInfoData, mRecyclerViewCarouselInfo, mMenuGroup, mPageTitle,mBgColor);
    }
    ...
    public void bindItemViewHolder(int position) {
        ...
        holder.see_all_text.setTag(carouselInfoData);
        ...
        if(holder.mTextViewViewAll!=null){
            ...
            holder.see_all_text.setOnClickListener(mViewAllClickListener);
        }
        ...
        if (carouselInfoData.title!=null&&!TextUtils.isEmpty(carouselInfoData.title)){
           // holder.see_all_text.setText("See all "+carouselInfoData.title);
            holder.see_all_text.setText("See More");
        }
...
//            }
        ...
        adapterBigBannerVerticalCarousel = null;
        ...
        if (holder.mRecyclerViewCarousel.getTag() instanceof AdapterBigBannerVerticalCarousel) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterBigBannerVerticalCarousel = (AdapterBigBannerVerticalCarousel) holder.mRecyclerViewCarousel.getTag();
            adapterBigBannerVerticalCarousel.setData(carouselInfoData.listCarouselData);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
            adapterBigBannerVerticalCarousel = new AdapterBigBannerVerticalCarousel(mContext, carouselInfoData.listCarouselData,holder.mRecyclerViewCarousel);
        }
        LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " requestState- " + carouselInfoData.requestState);
        adapterBigBannerVerticalCarousel.showTitle(carouselInfoData.showTitle);
        ...
        holder.mRecyclerViewCarousel.setTag(adapterBigBannerVerticalCarousel);
        adapterBigBannerVerticalCarousel.setOnItemClickListenerWithMovieData(mOnItemClickListenerMovies);
        adapterBigBannerVerticalCarousel.setParentViewHolder(holder);
        adapterBigBannerVerticalCarousel.setParentPosition(position);
        adapterBigBannerVerticalCarousel.setmBgColor(mBgColor);
        if (mListCarouselInfo.get(position).layoutType!=null){
            adapterBigBannerVerticalCarousel.setLayoutType(mListCarouselInfo.get(position).layoutType);
        }
        adapterBigBannerVerticalCarousel.setCarouselInfoData(carouselInfoData.name, carouselInfoData.pageSize);
        adapterBigBannerVerticalCarousel.isContinueWatchingSection(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_CONTINUE_WATCHING.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterBigBannerVerticalCarousel.isGenericLayout(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_HORIZONTAL_LIST_GENERIC_ITEM.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterBigBannerVerticalCarousel.setRemoveItemListener(mOnItemRemovedListener);
        holder.mRecyclerViewCarousel.setAdapter(adapterBigBannerVerticalCarousel);
    ...
    private class CarouselRequestTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(final Void... params) {
            ...
            new MenuDataModel().fetchCarouseldata(mContext, carouselInfoData.name, 1, APIConstants.PAGE_INDEX_COUNT, isCacheRequest, carouselInfoData.modified_on,new MenuDataModel.CarouselContentListCallback() {
                @Override
