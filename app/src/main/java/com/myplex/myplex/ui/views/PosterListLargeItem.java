...
public class PosterListLargeItem extends GenericListViewCompoment {
    ...
    AdapterBigHorizontalCarousel adapterBigHorizontalCarousel = null;
    private Typeface mBoldTypeFace;
    public PosterListLargeItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, String menuGroup, String pageTitle) {
        super(view);
        this.view = view;
    ...
    public static PosterListLargeItem createView(Context context, ViewGroup parent,
                                                 List<CarouselInfoData> carouselInfoData, RecyclerView mRecyclerViewCarouselInfo, String menuGroup, String pageTitle) {
        View view = LayoutInflater.from(context).inflate(R.layout.listitem_poster_recycler_view, parent, false);
        if (DeviceUtils.isTablet(context)) {
            DisplayMetrics dm = new DisplayMetrics();
            ((MainActivity) context).getWindowManager().getDefaultDisplay().getMetrics(dm);
            int height = (int) ((dm.widthPixels / dm.density) / 2.375f);
            if (context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
                height = (int) ((dm.heightPixels / dm.density) / 2.375f);
            }
            view.setMinimumHeight(height);
        }
        return new PosterListLargeItem(context, view, carouselInfoData, menuGroup, pageTitle);
    }

    @Override
    public void bindItemViewHolder(int position) {
        this.position = position;
        //Log.d(TAG, "bindPosterListLargeItemViewHolder");
        ...
        holder.mLayoutViewAllRelative.setTag(carouselInfoData);
        holder.mLayoutViewAllRelative.setOnClickListener(mViewAllClickListener);
        holder.mLayoutViewAllRelative.setVisibility(View.GONE);
        if (carouselInfoData.enableShowAll) {
            holder.mLayoutViewAllRelative.setVisibility(View.VISIBLE);
...
//        holder.mLayoutCarouselTitle.setBackgroundColor(UiUtil.getColor(mContext, R.color.app_theme_color));
        /*if (!TextUtils.isEmpty(carouselInfoData.bgColor)) {
            try {
                holder.mPosterRecyclerView.setBackgroundColor(Color.parseColor(carouselInfoData.bgColor));
        ...
        adapterBigHorizontalCarousel = null;
        if (carouselInfoData.listCarouselData == null) {
            carouselInfoData.listCarouselData = Util.getDummyCardData();
        }
        if (holder.mPosterRecyclerView.getTag() instanceof AdapterBigHorizontalCarousel) {
            adapterBigHorizontalCarousel = (AdapterBigHorizontalCarousel) holder.mPosterRecyclerView.getTag();
            ...
            adapterBigHorizontalCarousel = new AdapterBigHorizontalCarousel(mContext,
                    carouselInfoData.listCarouselData,holder.mPosterRecyclerView.mRecyclerView,mPageTitle);
        ...
        holder.mPosterRecyclerView.setTag(adapterBigHorizontalCarousel);
        holder.mPosterRecyclerView.setImageUrl(carouselInfoData.getPosterImageLink());
        adapterBigHorizontalCarousel.showTitle(false);
        holder.mPosterRecyclerView.setAdapter(adapterBigHorizontalCarousel);
        ...
        holder.mPosterRecyclerView.mRecyclerView.removeItemDecoration(mHorizontalMoviesDivieder);
        holder.mPosterRecyclerView.mRecyclerView.addItemDecoration(mHorizontalMoviesDivieder);
        ...
        switch (carouselInfoData.requestState) {
            case NOT_LOADED:
                ...
                holder.mPosterRecyclerView.setVisibility(View.VISIBLE);
            ...
            case ERROR:
                ...
                holder.mPosterRecyclerView.setVisibility(View.INVISIBLE);
