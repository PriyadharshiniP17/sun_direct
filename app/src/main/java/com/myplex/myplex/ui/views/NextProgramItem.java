...
import com.myplex.myplex.ui.adapter.AdapterNextProgramCarousel;
...
public class NextProgramItem extends GenericListViewCompoment {
    ...
    AdapterNextProgramCarousel adapterSmallHorizontalCarousel = null;
    String mBgColor;
    private boolean isFromSearch;
    private String globalServiceId;
    private int selectedTabIndex;
    private Typeface mBoldTypeFace;
    public NextProgramItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo
        ...
        mHorizontalDividerDecoration = new HorizontalItemDecorator((int) mContext
                .getResources().getDimension(R.dimen.margin_gap_3));
    ...
    public static NextProgramItem createView(Context context, ViewGroup parent,
        ...
        return new NextProgramItem(context, view, carouselInfoData, mRecyclerViewCarouselInfo, mMenuGroup,  mPageTitle,mBgColor, isFromSearch,globalServiceId);
    }
    ...
    public void bindItemViewHolder(int position) {
        ...
        List<EPG.EPGTab> epgTabList = Util.showNextDatesEPG(false);
        for (int i=0; i < epgTabList.size(); i++) {
            if(i == 0)
                tabLayout.addTab(tabLayout.newTab().setText(robotoStringFont("Up Next")));
            else
                tabLayout.addTab(tabLayout.newTab().setText(robotoStringFont(epgTabList.get(i).getTitle())));
        ...
        tabLayout.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabSelected(TabLayout.Tab tab) {
                ...
                holder.mRecyclerViewCarousel.scrollToPosition(0);
      ...
      //  holder.mLayoutViewAll.setVisibility(View.VISIBLE);
        ...
        if (holder.mRecyclerViewCarousel.getTag() instanceof AdapterNextProgramCarousel) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterSmallHorizontalCarousel = (AdapterNextProgramCarousel) holder.mRecyclerViewCarousel.getTag();
            adapterSmallHorizontalCarousel.setData(carouselInfoData.listCarouselData,0);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
            adapterSmallHorizontalCarousel = new AdapterNextProgramCarousel(mContext, carouselInfoData.listCarouselData, holder.mRecyclerViewCarousel);
        }
        if (carouselInfoData != null)
            adapterSmallHorizontalCarousel.showTitle(carouselInfoData.showTitle);
    ...
    private class CarouselRequestTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(final Void... params1) {
           ...
           /* new MenuDataModel().setPortraitBannerRequest(APIConstants.isPortraitBannerLayout(carouselInfoData)).fetchCarouseldata(mContext, carouselInfoData.name, 1, carouselInfoData.pageSize > 0 ? carouselInfoData.pageSize : APIConstants.PAGE_INDEX_COUNT, isCacheRequest, carouselInfoData.modified_on,new MenuDataModel.CarouselContentListCallback() {
            ...
            ApplicationController.DATE_POSITION = selectedTabIndex;
            if(selectedTabIndex == 0 ) {
                date = "";
                if (((MainActivity) mContext) != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer != null && (((MainActivity) mContext).mFragmentCardDetailsPlayer.mIsSupportCatchup ||
                ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer.isFromCatchup))
                    currentProgram = true;
            }
            else {
               // ApplicationController.DATE_POSITION = selectedTabIndex;
                final Date dateT = Util.getCurrentDate(ApplicationController.DATE_POSITION);
                SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                String selectedDateInString = format.format(dateT);
                date = Util.getYYYYMMDD(selectedDateInString);
            }
            ChannelListEPG.Params params = new ChannelListEPG.Params(globalServiceId, date , true, currentProgram);
            ChannelListEPG channelListEPG = new ChannelListEPG(params, new APICallback<ChannelsEPGResponseData>() {
                ...
                public void onResponse(APIResponse<ChannelsEPGResponseData> response) {
                    if(response != null && response.body() != null && response.body().getResults() != null && response.body().getResults().size() >0) {
                        addCarouselData(response.body().getResults().get(0).getPrograms(), carouselInfoData);
                    }
                }
    ...
    private void addCarouselData(final List<CardData> carouselList, final CarouselInfoData carouselInfoData) {
        if (carouselList == null && carouselList.size() == 0) {
            ...
            removeItemFromParent(carouselInfoData);
        } else if(carouselList != null && carouselList.size()<=0){
            ...
            adapterSmallHorizontalCarousel.setData(carouselInfoData.listCarouselData,selectedTabIndex);
        }
        else {
            LoggerD.debugLogAdapter("carousel empty results - " + carouselInfoData.title + " requestState- success");
            carouselInfoData.requestState = RequestState.SUCCESS;
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
    ...
    private void showDetailsFragment(CardData carouselData, CarouselInfoData carouselInfoData, int parentPosition) {
        ...
        args.putBoolean("isLiveNow", true);
