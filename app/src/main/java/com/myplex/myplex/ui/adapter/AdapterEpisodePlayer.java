...
public class AdapterEpisodePlayer extends RecyclerView.Adapter<AdapterEpisodePlayer.EpisodeViewHolder> implements FetchDownloadProgress.DownloadProgressStatus {
    private static final String TAG = AdapterEpisodePlayer.class.getSimpleName();
    ...
    private static final int SUBSCRIPTION_REQUEST = 0;
    private static final String PARAM_RELATED_CARD_DATA ="" ;
    private static final int PARAM_SUBSCRIPTION_TYPE_NONE =0 ;
    private final HandlerThread mHandlerThread;
    private final Handler mPollingThreadHandler;
    private Context mContext;
    private MiniCardVideoPlayer mMiniCardVideoPlayer;
...
//                ((BaseActivity)mContext).pushFragment(CardDetails.newInstance(args));
    ...
    public void addAll(List<CardData> cardDataList,boolean isToUpdate) {
        if (mListEpisodes == null || cardDataList == null) return;
        int currentSize = mListEpisodes.size();
        mListEpisodes.addAll(cardDataList);
        notifyItemRangeInserted(currentSize, mListEpisodes.size());
    }

    public void removeAllEpisodesOnChangingSeason() {
        if(mListEpisodes != null && mListEpisodes.size() > 0) {
            mListEpisodes.clear();
        }
    }
    ...
    Typeface amazonEmberBold;
        public AdapterEpisodePlayer(Context context, List<CardData> episodesList, MiniCardVideoPlayer miniCardVideoPlayer) {
        mContext = context;
        ...
        mMiniCardVideoPlayer = miniCardVideoPlayer;
    ...
    public EpisodeViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case ITEM_TYPE_LOADING:
                ...
                break;
            default:
                viewHolder = new EpisodeViewHolder(LayoutInflater.from(mContext).inflate(R.layout.listitem_related_vods_tvshows_player, parent, false));
    ...
    public void onBindViewHolder(final EpisodeViewHolder mViewHolder, int position) {
        contentId= PreferenceManager.getDefaultSharedPreferences(mContext).getString("contentTitle", "");
        ...
        if (cardData != null) {
            if (cardData.generalInfo != null) {
                if (cardData.isVODChannel()
                    ...
                    mViewHolder.mEpiosdeTitle.setTextColor(mContext.getResources().getColor(R.color.white));
                    mViewHolder.mEpisodeSNo.setTextColor(mContext.getResources().getColor(R.color.white));
                } else if (cardData.generalInfo.title != null) {
//here we are holding data
                    String[] splitText = cardData.generalInfo.title.split(" " + Pattern.quote("|") + " ");

                    if (splitText.length >= 2) {
                        LoggerD.debugDownload("splitText- " + splitText + "\n1. " + splitText[0] + " \n2. " + splitText[1]);
                       // holder.mTextViewVODInfo1.setText(splitText[0] + " | " + splitText[1]);
                        mViewHolder.mEpiosdeTitle.setText(splitText[2]);
                        mViewHolder.mEpiosdeTitle.setTypeface(amazonEmberBold);
                        mViewHolder.mEpisodeSNo.setText(splitText[1]);

                        if(cardData.generalInfo!=null && cardData.generalInfo._id!=null && contentId.equalsIgnoreCase(cardData.generalInfo._id)){
                            mViewHolder.mEpiosdeTitle.setTextColor(mContext.getResources().getColor(R.color.yellow_strip));
                            mViewHolder.mEpisodeSNo.setTextColor(mContext.getResources().getColor(R.color.yellow_strip));
                        }else{
                            mViewHolder.mEpiosdeTitle.setTextColor(mContext.getResources().getColor(R.color.white));
                            mViewHolder.mEpisodeSNo.setTextColor(mContext.getResources().getColor(R.color.white));                        }
        ...
        if (cardDownloadData != null) {
            if (cardDownloadData.mCompleted) {
                ...
                showDownloadingUI(mViewHolder, cardDownloadData);
                    ...
                    FetchDownloadProgress.getInstance(mContext).removeProgressListener(AdapterEpisodePlayer.this);
                    FetchDownloadProgress.getInstance(mContext).addProgressListener(AdapterEpisodePlayer.this);
...
//                                                }
    ...
    public class EpisodeViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        TextView mEpiosdeTitle, mEpisodeSNo;
        ...
        public EpisodeViewHolder(View itemView) {
            ...
            mEpiosodeDuration = (TextView) itemView.findViewById(R.id
                    .textview_duration_player);
            mEpiosdeTitle = (TextView) itemView.findViewById(R.id
                    .textview_title_player);
            mEpisodeSNo = (TextView) itemView.findViewById(R.id
                    .textview_searial_no);
...
/*
    public void setEpisodeData(CardData cardData){
...
*/
    ...
    private void showDetailsFragment(CardData cardData) {
        ...
        if(cardData.generalInfo!=null && cardData.generalInfo._id!=null) {
            SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(mContext);
            SharedPreferences.Editor editor = preferences.edit();
            editor.putString("contentTitle", cardData.generalInfo._id);
            editor.commit();
        }
        ...
        ((MiniCardVideoPlayer) mMiniCardVideoPlayer).playNextItem_player(args, cardData);
