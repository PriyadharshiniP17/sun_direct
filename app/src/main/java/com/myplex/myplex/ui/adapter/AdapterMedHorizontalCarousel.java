...
public class AdapterMedHorizontalCarousel extends RecyclerView.Adapter<AdapterMedHorizontalCarousel.CarouselDataViewHolder>{

    private static final String TAG = AdapterMedHorizontalCarousel.class.getSimpleName();
...
//    private List<CarouselInfoData> mListCarouselInfo;
    ...
    public AdapterMedHorizontalCarousel(Context context, List<CardData> itemList,RecyclerView recyclerView) {
        mContext = context;
    ...
    public AdapterMedHorizontalCarousel(Context context, List<CardData> itemList) {
        mContext = context;
        mListMovies = itemList;
        mBoldTypeFace = Typeface.createFromAsset(mContext.getAssets(), "font/amazon_ember_cd_bold.ttf");
...
//    }
    ...
    public CarouselDataViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        view = LayoutInflater.from(mContext).inflate(R.layout.listitem_carousel_medium_recycler, parent, false);
       ...
       /* if(carouselData != null && carouselData.generalInfo !=null && carouselData.generalInfo.contentRights != null && carouselData.generalInfo.contentRights.size()>0 && carouselData.generalInfo.contentRights.get(0)!= null) {
        ...
        if (!TextUtils.isEmpty(partnerImageLink)) {
            ...
            PicassoUtil.with(mContext)
                    .loadCenterInside(partnerImageLink,holder.mImageViewPartner,R.drawable.black,holder.mImageViewPartner.getLayoutParams().width, holder.mImageViewPartner.getLayoutParams().height);
        }else{
...
//            mImageViewRupeeIcon = (ImageView)view.findViewById(R.id.thumbnail_rupee_icon);
            ...
            layoutContainer.getLayoutParams().width = itemView.getResources().getDimensionPixelSize(R.dimen.medium_horizontal_width);
            mImageViewMovies.getLayoutParams().width = itemView.getResources().getDimensionPixelSize(R.dimen.medium_horizontal_width);


           // layoutContainer.getLayoutParams().height = itemView.getResources().getDimensionPixelSize(R.dimen.rp_max_width);
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
    ...
    public void showViewRatingOnThumbnail(AdapterMedHorizontalCarousel.CarouselDataViewHolder holder, CardData cardData){
