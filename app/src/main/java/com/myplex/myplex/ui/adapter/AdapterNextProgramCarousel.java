...
public class AdapterNextProgramCarousel extends RecyclerView.Adapter<AdapterNextProgramCarousel.CarouselDataViewHolder> {

    private static final String TAG = AdapterNextProgramCarousel.class.getSimpleName();
...
//    private boolean mIsDummyData = false;
    ...
    public AdapterNextProgramCarousel(Context context, List<CardData> itemList) {
        mContext = context;
        mListMovies = itemList;
//        ScopedBus.getInstance().register(this);
    }
    public AdapterNextProgramCarousel(Context context, List<CardData> itemList, RecyclerView recyclerView) {
        mContext = context;
        mListMovies = itemList;
...
*/
    ...
    public void setData(final List<CardData> listMovies, int selectTab) {
    ...
    public CarouselDataViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        view = LayoutInflater.from(mContext).inflate(R.layout.next_program_carousel, parent, false);
    ...
    public void onBindViewHolder(CarouselDataViewHolder holder, int position) {
        ...
        bindGenreViewHolder(holder, mListMovies.get(position), position);
    }


    private void bindGenreViewHolder(final CarouselDataViewHolder holder, final CardData carouselData, int mainPosition) {
...
//                        S2 | E1 | Pal Pal Dil Ke Paas
                        ...
                        if (carouselData.isMovie()) {
                            ...
                            if (carouselData.isTVEpisode()) {
                                try {
                                    ...
                                    if (!TextUtils.isEmpty(carouselData.getTitle())) {
                                        holder.mTextViewVODInfo1.setVisibility(View.VISIBLE);
                                        holder.mTextViewVODInfo1.setText(carouselData.getTitle());
                                    }
            ...
            ReminderListener reminderListener = new ReminderListener(mContext, null, ApplicationController.DATE_POSITION);
      /*  holder.reminderLL.setOnClickListener(new View.OnClickListener() {
            ...
            holder.mReminderImage.setColorFilter(SDKUtils.getColor(mContext,R.color.white), PorterDuff.Mode.MULTIPLY);
            if(alarmData != null
                    && alarmData.title != null
                    && alarmData.title.equalsIgnoreCase(carouselData.generalInfo.title)){
                holder.mReminderImage.setColorFilter(SDKUtils.getColor(mContext,R.color.theme_app_color), PorterDuff.Mode.MULTIPLY);
            }
            /*if (carouselData != null
        ...
        holder.mReminderImage.setVisibility(View.VISIBLE);
        showTitle = false;
        try {
            if(carouselData.startDate != null && carouselData.endDate != null) {
                ...
                if(mainPosition == selectedTabIndex) {
                    if (((MainActivity) mContext) != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer.isFromCatchup) {
                        holder.mTimeTv.setText("Live Now");
                        holder.mTimeTv.setTextColor(mContext.getResources().getColor(R.color.theme_app_color));
                        holder.mReminderImage.setVisibility(View.GONE);
                    }
                    else {
                        holder.mTimeTv.setText(startTime + " - " + endTime);
                        holder.mTimeTv.setTextColor(mContext.getResources().getColor(R.color.title_text_color));

                    }

//                    holder.mTimeTv.setTypeface(mRegularTypeFace);
                }
                else {
                    holder.mTimeTv.setText(startTime + " - " + endTime);
                    holder.mTimeTv.setTextColor(mContext.getResources().getColor(R.color.title_text_color));
//                    holder.mTimeTv.setTypeface(mBoldTypeFace);
                }
...
//        return type;
    ...
    public class CarouselDataViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        final ImageView mImageViewMovies, mReminderImage;
...
//            mImageViewRupeeIcon = (ImageView)view.findViewById(R.id.thumbnail_rupee_icon);
            ...
            mReminderImage = view.findViewById(R.id.reminder_image);
//
        ...
        public void onClick(View v) {
            if (this.clickListener != null && mListMovies != null && getAdapterPosition() != -1) {
                if(selectedTabIndex == getAdapterPosition()) {
                    if (((MainActivity) mContext) != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer != null && ((MainActivity) mContext).mFragmentCardDetailsPlayer.mPlayer.isFromCatchup) {
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
    ...
    public void showViewRatingOnThumbnail(AdapterNextProgramCarousel.CarouselDataViewHolder holder, CardData cardData){
