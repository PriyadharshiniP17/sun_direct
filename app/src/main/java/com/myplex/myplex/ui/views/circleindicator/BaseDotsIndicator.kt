...
abstract class BaseDotsIndicator @JvmOverloads constructor(context: Context,
    ...
    fun setViewPager(viewPager: ViewPager,dotsCount:Int) {
    ...
    fun setRecyclerView(recyclerView: RecyclerView, snapHelper: SnapHelper,count: Int) {


        mRecyclerView = recyclerView
        mSnapHelper = snapHelper
        System.out.println("INDICATORS COUNT:: "+ count)
        ...
        pager = object : Pager {
            ...
            override val isNotEmpty: Boolean
                get() = recyclerView.isNotEmpty
            override val currentItem: Int
                get() = getSnapPosition(recyclerView.layoutManager)
            override val isEmpty: Boolean
                get() = recyclerView.isEmpty
            override val count: Int
                get() = count
    ...
    private val mInternalOnScrollListener: RecyclerView.OnScrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            val position: Int = getSnapPosition(recyclerView.layoutManager)
            if (position == RecyclerView.NO_POSITION) {
                return
            }
            if (position <= pager!!.count) {
                refreshDots()
            }
        }
    }

    open fun getSnapPosition(layoutManager: RecyclerView.LayoutManager?): Int {
    ...
    protected val RecyclerView.isNotEmpty: Boolean get() = adapter!!.itemCount > 0
    ...
    protected val RecyclerView?.isEmpty: Boolean
        get() = this != null && this.adapter != null &&
                adapter!!.itemCount == 0
