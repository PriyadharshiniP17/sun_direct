...
public class QualityTrackAdapter extends RecyclerView.Adapter<QualityTrackAdapter.ViewHolder>{
    private List<SelectQuality> listdata;
    private MediaController2.AudioTrackListner audioTrackListner;
    private  int audioPosition;
    private Context mContext;

    // RecyclerView recyclerView;
    public QualityTrackAdapter(List<SelectQuality> listdata, MediaController2.AudioTrackListner audioTrackListner, int audioPosition, Context context) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        final SelectQuality myListData = listdata.get(position);
        holder.textView.setText(LangUtil.getSubtitleTrackName(myListData.getQuality()));
