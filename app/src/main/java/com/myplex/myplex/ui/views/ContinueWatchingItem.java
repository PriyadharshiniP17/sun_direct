...
import com.myplex.myplex.ui.adapter.ContinueWatchingHorizontalCarousel;
...
public class ContinueWatchingItem extends GenericListViewCompoment implements ContinueVODPlayerCallback {
    ...
    private ContinueWatchingHorizontalCarousel adapterContinueWatchingCarousel = null;
    private Typeface mBoldTypeFace;

    public ContinueWatchingItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo, String menuGroup, String pageTitle) {
        super(view);
        this.view = view;
      ...
      /*  int margin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (int) mContext
        ...
        ((MainActivity) mContext).setVODCardPlayerCallback(this);
    }

    public static ContinueWatchingItem createView(Context context, ViewGroup parent,
        ...
        return new ContinueWatchingItem(context, view, carouselInfoData, mRecyclerViewCarouselInfo, menuGroup, pageTitle);
    }
...
//            }
        ...
        adapterContinueWatchingCarousel = null;
        ...
        if (holder.mRecyclerViewCarousel.getTag() instanceof ContinueWatchingHorizontalCarousel) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterContinueWatchingCarousel = (ContinueWatchingHorizontalCarousel) holder.mRecyclerViewCarousel.getTag();
            adapterContinueWatchingCarousel.setData(carouselInfoData.listCarouselData);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
            adapterContinueWatchingCarousel = new ContinueWatchingHorizontalCarousel(mContext, carouselInfoData.listCarouselData, holder.mRecyclerViewCarousel);
        }
        if (carouselInfoData != null)
            adapterContinueWatchingCarousel.showTitle(carouselInfoData.showTitle);
        ...
        holder.mRecyclerViewCarousel.setTag(adapterContinueWatchingCarousel);
        adapterContinueWatchingCarousel.setOnItemClickListenerWithMovieData(mOnItemClickListenerMovies);
        adapterContinueWatchingCarousel.setParentPosition(position);
        adapterContinueWatchingCarousel.setParentViewHolder(holder);
        adapterContinueWatchingCarousel.setCarouselInfoData(carouselInfoData.name, carouselInfoData.pageSize);
        adapterContinueWatchingCarousel.setTextureData(carouselInfoData.texture);
        adapterContinueWatchingCarousel.isContinueWatchingSection(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_CONTINUE_WATCHING.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterContinueWatchingCarousel.setRemoveItemListener(mOnItemRemovedListener);
        holder.mRecyclerViewCarousel.setAdapter(adapterContinueWatchingCarousel);
    ...
    public void onDocPlayerClosed() {
        if (carouselInfoData != null && carouselInfoData.listCarouselData != null && carouselInfoData.listCarouselData.size()>0) {
            ...
            if (mRecyclerViewCarouselInfo != null && mRecyclerViewCarouselInfo.getAdapter() != null) {
                ((AdapterCarouselInfo) mRecyclerViewCarouselInfo.getAdapter()).safelyNotifyItemAdded(carouselInfoData, position);
            }
        }
    }
    ...
    private void addCarouselData(final List<CardData> carouselList, final CarouselInfoData carouselInfoData) {
        if (carouselList == null || carouselList.size() == 0) {
            ...
            if (carouselInfoData != null && carouselInfoData.listCarouselData != null) {
                if (carouselInfoData.listCarouselData.size() == 10) {
    ...
    public final ItemClickListenerWithData mOnItemClickListenerMovies = new ItemClickListenerWithData() {
        ...
        public void onClick(View view, int position, int parentPosition, final CardData carouselData) {
            ...
            ApplicationController.IS_FROM_CONTINUE_WATCHING=true;
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
            ...
            if(carouselData.isTVSeries() || carouselData.isTVEpisode()){
                new CacheManager().getCardDetails(carouselData.globalServiceId, false, new CacheManager.CacheManagerCallback() {
                    @Override
                    public void OnCacheResults(List<CardData> dataList) {
                        if (dataList == null || dataList.size() == 0) {
                            return;
                        }
                        showDetailsFragment(carouselData, dataList.get(0).globalServiceId);
                    }
                    ...
                    @Override
                    public void OnOnlineError(Throwable error, int errorCode) {
                        Toast.makeText(mContext, errorCode + " : " +error.getLocalizedMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
            else {
                if (carouselData.globalServiceId != null)
                    getEPGData(carouselData.globalServiceId,carouselData);
                else if (carouselData._id != null) {
                    if (carouselData.isLive()) {
                        getEPGData(carouselData._id,carouselData);
                    } else {
    ...
    public void getEPGData(String contentId,CardData cardData){
...
*/
    private void showDetailsFragment(CardData carouselData, String seasonGlobalServiceId) {
...
*/
    ...
    private void removeItemFromParent(CarouselInfoData carouselInfoData, int mParentPosition) {
        ...
        if (mRecyclerViewCarouselInfo != null && mRecyclerViewCarouselInfo.getAdapter() != null) {
            ((AdapterCarouselInfo) mRecyclerViewCarouselInfo.getAdapter()).safelyNotifyItemRemoved(carouselInfoData);
        }
    }
