...
public class AdapterCategoryNestedCarousel extends RecyclerView.Adapter<AdapterCategoryNestedCarousel.CarouselDataViewHolder> {

    private static final String TAG = AdapterCategoryNestedCarousel.class.getSimpleName();
    ...
    NestedCarouselItem.CategoryOnClick mCategoryOnClick;
    //adapter view click listener

    public AdapterCategoryNestedCarousel(Context context, CarouselInfoData carouselInfoData, RecyclerView recyclerView, NestedCarouselItem.CategoryOnClick categoryOnClick) {
        mContext = context;
        ...
        mCategoryOnClick = categoryOnClick;
...
//                Util.updatePlayerStatus(0, Analytics.ACTION_TYPES.delete.name(), mListMovies.get(pos)._id,mListMovies.get(pos).getType());
    ...
    public CarouselDataViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.category_listitem_nested_carousel_item, parent, false);
...
//            public void run() {
        ...
        holder.mName.setText(carouselData.name);
        if(carouselData.isSelected == true)
        {
            holder.mName.setBackgroundColor(mContext.getResources().getColor(R.color.yellow));
            holder.mName.setTextColor(mContext.getResources().getColor(R.color.black));

        } else {
            holder.mName.setBackgroundColor(mContext.getResources().getColor(R.color.black));
        ...
        holder.setCategoryClick(mCategoryOnClick);
...
//        return type;
    ...
    class CarouselDataViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private NestedCarouselItem.CategoryOnClick mCategoryClick;
...
//            UiUtil.showFeedback(view, true, R.color.list_item_bkg);
        ...
        public void setCategoryClick(NestedCarouselItem.CategoryOnClick categoryClick) {
            this.mCategoryClick = categoryClick;
        }

        @Override
        public void onClick(View v) {
            if (this.mCategoryClick != null && mListMovies != null && getAdapterPosition() != -1) {
              /*  mListMovies.get(mParentPosition).isSelected = true;
                notifyDataSetChanged();*/
                this.mCategoryClick.categoryOnClick(v, getAdapterPosition(), mParentPosition, mListMovies.get(getAdapterPosition()));
