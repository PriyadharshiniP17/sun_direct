...
import com.myplex.myplex.ui.adapter.AdapterPartnerHorizontalCarousel;
...
public class PartnerHorizontalItem extends GenericListViewCompoment {
    ...
    AdapterPartnerHorizontalCarousel adapterRoundedArtistHorizontalCarousel = null;
    private Typeface mBoldTypeFace;
    public PartnerHorizontalItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo, String mMenuGroup, String mPageTitle,boolean isFromSearch) {
        super(view);
        this.view = view;
        this.mContext = mContext;
        this.mListCarouselInfo = mListCarouselInfo;
        mHorizontalDividerDecoration = new HorizontalItemDecorator((int) mContext
                .getResources().getDimension(R.dimen.margin_gap_1));
    ...
    /*    int margin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (int) mContext
        ...
        int margin = (int) mContext.getResources().getDimension(R.dimen.margin_gap_3);
    ...
    public static PartnerHorizontalItem createView(Context context, ViewGroup parent,
                                                   List<CarouselInfoData> carouselInfoData, RecyclerView mRecyclerViewCarouselInfo, String mMenuGroup, String mPageTitle,boolean isFromSearch) {
        View view = LayoutInflater.from(context).inflate(R.layout.listitem_carousel_linear_recycler, parent, false);
        return new PartnerHorizontalItem(context, view, carouselInfoData, mRecyclerViewCarouselInfo, mMenuGroup, mPageTitle,isFromSearch);
    }
...
//            }
        ...
        adapterRoundedArtistHorizontalCarousel = null;
        ...
        if (holder.mRecyclerViewCarousel.getTag() instanceof AdapterMedHorizontalCarousel) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterRoundedArtistHorizontalCarousel = (AdapterPartnerHorizontalCarousel) holder.mRecyclerViewCarousel.getTag();
            adapterRoundedArtistHorizontalCarousel.setData(carouselInfoData.listCarouselData);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
            adapterRoundedArtistHorizontalCarousel = new AdapterPartnerHorizontalCarousel(mContext, carouselInfoData.listCarouselData,holder.mRecyclerViewCarousel,isFromSearch);
        }
        LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " requestState- " + carouselInfoData.requestState);
        adapterRoundedArtistHorizontalCarousel.showTitle(carouselInfoData.showTitle);
        ...
        holder.mRecyclerViewCarousel.setTag(adapterRoundedArtistHorizontalCarousel);
        adapterRoundedArtistHorizontalCarousel.setOnItemClickListenerWithMovieData(mOnItemClickListenerMovies);
        adapterRoundedArtistHorizontalCarousel.setParentViewHolder(holder);
        adapterRoundedArtistHorizontalCarousel.setParentPosition(position);
        adapterRoundedArtistHorizontalCarousel.setCarouselInfoData(carouselInfoData.name, carouselInfoData.pageSize);
        adapterRoundedArtistHorizontalCarousel.isContinueWatchingSection(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_CONTINUE_WATCHING.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterRoundedArtistHorizontalCarousel.isGenericLayout(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_HORIZONTAL_LIST_GENERIC_ITEM.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterRoundedArtistHorizontalCarousel.setRemoveItemListener(mOnItemRemovedListener);
        holder.mRecyclerViewCarousel.setAdapter(adapterRoundedArtistHorizontalCarousel);
    ...
    public final ItemClickListenerWithData mOnItemClickListenerMovies = new ItemClickListenerWithData() {
        ...
        public void onClick(View view, int position, int parentPosition, final CardData carouselData) {
            ...
            CarouselInfoData carouselInfoData = parentPosition >= 0 ? mListCarouselInfo.get(parentPosition) : null;
             ...
             if(/*!isFromSearch &&*/ carouselData != null && carouselData.content != null && carouselData.content.actionURL != null) {
                 ((MainActivity) mContext).setAppsTab(carouselData.content.actionURL);
                 return;
             }
            if (carouselInfoData != null && carouselInfoData.name != null) {
                gaBrowse(carouselData, carouselInfoData.name);
                //Log.d(TAG, "carouselSectionName: " + carouselInfoData.name);
            }
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
    ...
    private void showDetailsFragment(CardData carouselData, CarouselInfoData carouselInfoData, int parentPosition) {
        ...
        if (carouselData != null
                && carouselData.generalInfo != null) {
         /*   if(APIConstants.TYPE_PROGRAM.equalsIgnoreCase(carouselData.generalInfo.type) || APIConstants.TYPE_LIVE.equalsIgnoreCase(carouselData.generalInfo.type)) {
    ...
    private final View.OnClickListener mViewAllClickListener = new View.OnClickListener() {
        ...
        public void onClick(final View view) {
           /* if(*//*!isFromSearch*//*){*/
            ((MainActivity) mContext).setAppsTab("");
           /* return;}
            else {
                return;
            }*/
