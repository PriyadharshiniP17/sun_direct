...
import com.myplex.myplex.ui.adapter.AdapterNestedCarousel;
...
public class NestedCarouselItem extends GenericListViewCompoment {
    ...
    AdapterNestedCarousel adapterNestedCarousel = null;
    private RecyclerView recycler_view_list;
    private Typeface mBoldTypeFace;
    public NestedCarouselItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo, String menuGroup, String pageTitle) {
        super(view);
        this.view = view;
        ...
        layoutManager = new CustomLinearLayoutManager(mContext, LinearLayoutManager.VERTICAL, false);
        layoutManager.setItemPrefetchEnabled(false);
        recycler_view_list.addItemDecoration(mHorizontalDividerDecoration);
        recycler_view_list.setItemAnimator(null);
    ...
    public static NestedCarouselItem createView(Context context, ViewGroup parent,
                                                List<CarouselInfoData> carouselInfoData, RecyclerView mRecyclerViewCarouselInfo, String menuGroup, String pageTitle) {
        View view = LayoutInflater.from(context).inflate(R.layout.listitem_carousel_tab, parent, false);
        return new NestedCarouselItem(context, view, carouselInfoData, mRecyclerViewCarouselInfo, menuGroup,  pageTitle);
    }
...
//            }
        ...
        if (carouselInfoData.listNestedCarouselInfoData == null) {
            ...
            carouselInfoData.listNestedCarouselInfoData = Util.getDummyCarouselInfoData();
        }
        if (holder.mRecyclerViewCarousel.getTag() instanceof AdapterNestedCarousel) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterNestedCarousel = (AdapterNestedCarousel) holder.mRecyclerViewCarousel.getTag();
            adapterNestedCarousel.setData(carouselInfoData.listNestedCarouselInfoData);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
          //  adapterNestedCarousel = new AdapterNestedCarousel(mContext, carouselInfoData,holder.mRecyclerViewCarousel, mCategoryClick);
            adapterNestedCarousel = new AdapterNestedCarousel(mContext, carouselInfoData,holder.mRecyclerViewCarousel);
        }
        if (carouselInfoData != null)
            adapterNestedCarousel.showTitle(carouselInfoData.showTitle);
        ...
        holder.mRecyclerViewCarousel.setTag(adapterNestedCarousel);
        /*adapterNestedCarousel.setOnItemClickListenerWithMovieData(new AdapterNestedCarousel.ItemClickListenerWithData() {
            @Override
            public void onClick(View view, int position, int parentPosition, CarouselInfoData carouselData) {
        ...
        adapterNestedCarousel.setParentPosition(position);
        adapterNestedCarousel.setParentViewHolder(holder);
        adapterNestedCarousel.setCarouselInfoData(carouselInfoData.name, carouselInfoData.pageSize);
        adapterNestedCarousel.isContinueWatchingSection(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_CONTINUE_WATCHING.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterNestedCarousel.setRemoveItemListener(mOnItemRemovedListener);
        holder.mRecyclerViewCarousel.setAdapter(adapterNestedCarousel);
        ...
        switch (carouselInfoData.requestState) {
            case NOT_LOADED:
                ...
                startAsyncTaskInParallel(new CarouselInfoRequestTask(carouselInfoData));
                break;
            case IN_PROGRESS:
                holder.mTextViewErrorRetry.setVisibility(View.GONE);
                holder.mRecyclerViewCarousel.setVisibility(View.VISIBLE);
            ...
            case SUCCESS:
                if (carouselInfoData != null
                        && (carouselInfoData.listNestedCarouselInfoData == null
                        || carouselInfoData.listNestedCarouselInfoData.isEmpty())) {
    ...
    private void startAsyncTaskInParallel(CarouselInfoRequestTask task) {
    ...
    private class CarouselRequestTask1 extends AsyncTask<Void, Void, Void> {
        ...
        public CarouselRequestTask1(String pageName, int count, int position, String modifiedOn) {
            mPageName = pageName;
            mPosition = position;
            mCount = count;
                    ...
                    if (dataList != null && !dataList.isEmpty()) {
                        ...
                        ((BaseActivity) mContext).runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                recycler_view_list.setAdapter(new MyRecyclerViewAdapter(mContext, dataList));
                            }
                        });
                    }
                }
    ...
    private class CarouselInfoRequestTask extends AsyncTask<Void, Void, Void> {
        /**
         * Override this method to perform a computation on a background thread. The
         * specified parameters are the parameters passed to {@link #execute}
        ...
        public CarouselInfoRequestTask(CarouselInfoData carouselInfoData) {
            this.carouselInfoData = carouselInfoData;
        }

        @Override
        protected Void doInBackground(final Void... params) {
            new MenuDataModel().fetchMenuList(carouselInfoData.name, 1, APIConstants.PARAM_CAROUSEL_API_VERSION, new MenuDataModel.MenuDataModelCallback() {
                @Override
                public void onCacheResults(List<CarouselInfoData> dataList) {
                    addCarouselDataForNestedData(dataList, carouselInfoData);
                }
                ...
                @Override
                public void onOnlineError(Throwable error, int errorCode) {
                    addCarouselDataForNestedData(null, carouselInfoData);
                }
    ...
    private void addCarouselDataForNestedData(final List<CarouselInfoData> carouselList, final CarouselInfoData carouselInfoData) {
        if (carouselList == null || carouselList.size() == 0) {
            ...
            carouselInfoData.listNestedCarouselInfoData = carouselList;
        }
        for(int i =0 ;i <  carouselInfoData.listNestedCarouselInfoData.size() ; i++) {
            if ( i == 0 )
                carouselInfoData.listNestedCarouselInfoData.get(0).isSelected = true;
            else
                carouselInfoData.listNestedCarouselInfoData.get(0).isSelected = false;
        }

        adapterNestedCarousel.setData(carouselInfoData.listNestedCarouselInfoData);
        CarouselInfoData carouselData = carouselInfoData.listNestedCarouselInfoData.get(0);
        startAsyncTaskInParallel(new CarouselRequestTask1(carouselData.name, carouselData.pageSize > 0 ? carouselData.pageSize : APIConstants.PAGE_INDEX_COUNT, position, carouselData.modified_on));
    }
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
    ...
    public interface  CategoryOnClick {
        public void categoryOnClick(View view, int position, int parentPosition, CarouselInfoData carouselData);
    }

    public final CategoryOnClick mCategoryClick = new CategoryOnClick() {
        @Override
        public void categoryOnClick(View view, int position, int parentPosition, CarouselInfoData carouselData) {
