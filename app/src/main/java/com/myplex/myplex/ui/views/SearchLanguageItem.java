...
public class SearchLanguageItem extends GenericListViewCompoment{
    private List<CarouselInfoData> mListCarouselInfo;
    private Context mContext;
    private static final String TAG = SearchLanguageItem.class.getSimpleName();
    ...
    private AdapterSearchLanguagesItem adapterSearchLanguagesItem = null;
    String mBgColor;
    private boolean isFromSearch;
    private String globalServiceId;
    private Typeface mBoldTypeFace;

    public SearchLanguageItem(Context mContext, View view, List<CarouselInfoData> mListCarouselInfo, RecyclerView mRecyclerViewCarouselInfo
            , String mMenuGroup, String mPageTitle, String mBgColor, boolean isFromSearch, String globalServiceId) {
        super(view);
        this.view = view;
    ...
    public static SearchLanguageItem createView(Context context, ViewGroup parent,
                                                List<CarouselInfoData> carouselInfoData, RecyclerView mRecyclerViewCarouselInfo, String mMenuGroup,
                                                String mPageTitle, String mBgColor, boolean isFromSearch, String globalServiceId){
        View view = LayoutInflater.from(context).inflate(R.layout.listitem_carousel_search, parent, false);
        return new SearchLanguageItem(context, view, carouselInfoData, mRecyclerViewCarouselInfo, mMenuGroup,  mPageTitle,mBgColor, isFromSearch, globalServiceId);
    }
    ...
    public void bindItemViewHolder(int position) {
        ...
        if (mListCarouselInfo == null || mListCarouselInfo.isEmpty() || (position >= mListCarouselInfo.size())) {
            return;
        }
        ...
        adapterSearchLanguagesItem = null;
        ...
        if (holder.mRecyclerViewCarousel.getTag() instanceof AdapterSearchLanguagesItem) {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " getTag");
            adapterSearchLanguagesItem = (AdapterSearchLanguagesItem) holder.mRecyclerViewCarousel.getTag();
            adapterSearchLanguagesItem.setData(carouselInfoData.listCarouselData);
        } else {
            LoggerD.debugLogAdapter("carousel title- " + carouselInfoData.title + " create adapter");
            adapterSearchLanguagesItem = new AdapterSearchLanguagesItem(mContext, carouselInfoData.listCarouselData, holder.mRecyclerViewCarousel);
        }
        if (carouselInfoData != null)
            adapterSearchLanguagesItem.showTitle(carouselInfoData.showTitle);
        ...
        holder.mRecyclerViewCarousel.setTag(adapterSearchLanguagesItem);
        adapterSearchLanguagesItem.setOnItemClickListenerWithMovieData(mOnItemClickListenerMovies);
        adapterSearchLanguagesItem.setParentPosition(position);
        adapterSearchLanguagesItem.setParentViewHolder(holder);
        adapterSearchLanguagesItem.setBgColor(mBgColor);
        adapterSearchLanguagesItem.setCarouselInfoData(carouselInfoData.name, carouselInfoData.pageSize);
        adapterSearchLanguagesItem.isContinueWatchingSection(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_CONTINUE_WATCHING.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterSearchLanguagesItem.isGenericLayout(carouselInfoData.layoutType != null
                && APIConstants.LAYOUT_TYPE_HORIZONTAL_LIST_GENERIC_ITEM.equalsIgnoreCase(carouselInfoData.layoutType));
        adapterSearchLanguagesItem.setRemoveItemListener(mOnItemRemovedListener);
        holder.mRecyclerViewCarousel.setAdapter(adapterSearchLanguagesItem);
...
//                carouselData.generalInfo.deepLink = "http://www.sonyliv.com/details/live/5230736998001/Santhiya-Sehaj-Path-:-Giani-Jagtar-Singh-:-Epi-112";
       ...
       /*     if (carouselInfoData == null) {
          ...
          /*  if (carouselPosition >= 0) {
            ...
            if (!TextUtils.isEmpty(carouselData.content.actionURL)) {
                args.putString(APIConstants.ACTION_URL, carouselData.content.actionURL);
                PrefUtils.getInstance().setAppLanguageToShow(carouselData.generalInfo.title);
                ...
                ((MainActivity) mContext).pushFragment(FragmentLanguageCarouselInfo.newInstance(args));
            }
        }
    ...
    private void showDetailsFragment(CardData carouselData, CarouselInfoData carouselInfoData, int parentPosition) {
        ...
        args.putBoolean(CardDetails.PARAM_SUPPORT_CATCHUP, true);
