...
public class PinEntryEditText extends EditText {
    ...
    int[] mColors = new int[]{
            R.color.player_logs_title_text_color,
            R.color.player_logs_title_text_color,
            R.color.player_logs_title_text_color
    };
    ...
    protected void onDraw(Canvas canvas) {
        ...
        StringBuilder asteriskStr = null;
        for (int i = 0; i < textLength; i++) {
            if (asteriskStr == null) {
                asteriskStr = new StringBuilder("*");
            } else {
                asteriskStr.append("*");
            }
        }
        SDKLogger.debug("asteriskStr- " + asteriskStr + " textLength- " + textLength + " asteriskStr- " + asteriskStr + " text- " + text.toString());
        for (int i = 0; i < mNumChars; i++) {
            ...
            if (getText().length() > i) {
                ...
                if(mContext != null) {
                    getPaint().setColor(mContext.getResources().getColor(R.color.player_logs__text_color));
                }else{
                    getPaint().setColor(Color.WHITE);
                }
                SDKLogger.debug("mNumChars- " + mNumChars);
                if (isSecured)
                    canvas.drawText(asteriskStr.toString(), i, i + 1, middle - textWidths[0] / 2, bottom - mLineSpacing, getPaint());
                else
