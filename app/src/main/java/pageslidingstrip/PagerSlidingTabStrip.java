...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTabLayoutParams = isExpandTabs ?
         ...
         mFooterIndicatorHeight = getResources().getDimension(R.dimen.title_indicator_footer_indicator_height);
    }
...
//            canvas.drawRect(lines.first + mPaddingLeft, height - mIndicatorHeight, lines.second + mPaddingLeft, height, mRectPaint);
            ...
            float heightMinusLine = height - mUnderlineHeight;
            float footerIndicatorLineHeight = mFooterIndicatorHeight;


            mPaintFooterIndicator.setStyle(Paint.Style.FILL_AND_STROKE);
            mPaintFooterIndicator.setColor(mIndicatorColor);

            mPath.reset();
            float xArrow = (lines.first + mPaddingLeft) + (lines.second - lines.first)/2 ;
            mPath.moveTo(xArrow,  height - mIndicatorHeight);
            mPath.lineTo(xArrow + footerIndicatorLineHeight, heightMinusLine);
            mPath.lineTo(xArrow - footerIndicatorLineHeight, heightMinusLine);
            mPath.close();
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            ...
            tab_title.setTextColor(getResources().getColor(R.color.white_50));
            if (tab_title != null) {
    ...
    private void select(View tab) {
        if (tab != null) {
            TextView tab_title = (TextView) tab.findViewById(R.id.psts_tab_title);
            tab_title.setTextColor(getResources().getColor(R.color.white_100));
            if (tab_title != null) {
