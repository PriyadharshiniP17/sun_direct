...
class CarouselLayoutManager constructor(
    ...
    private fun layoutItem(child: View, rect: Rect) {
        ...
        if (!isFlat) {
            ...
            child.scaleY = computeScaleY(getRectLeftOrTop(rect) - mOffsetAll)
        }
    ...
    private fun computeScaleY(x: Int): Float {
        var scale: Float =
                1 - abs(x - mStartX) * 1.0f / abs(mStartX + mItemDecoratedWidth / (intervalRatio / 6))
