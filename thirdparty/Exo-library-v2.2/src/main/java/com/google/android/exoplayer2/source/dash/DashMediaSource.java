...
import com.google.android.exoplayer2.debug.LoggerD;
...
public final class DashMediaSource implements MediaSource {
  ...
  private void startLoadingManifest() {
    ...
    ParsingLoadable<DashManifest> parsingLoader = new ParsingLoadable<>(dataSource, manifestUri, C.DATA_TYPE_MANIFEST,
            manifestParser);
    LoggerD.debugLog("Dash videoUrl- " + videoUrl + " audioUrl- " + audioUrl);
    parsingLoader.setVideoUrl(videoUrl);
    parsingLoader.setAudioUrl(audioUrl);
    startLoading(parsingLoader, manifestCallback, minLoadableRetryCount);
  }
  ...
  private void resolveUtcTimingElementHttp(UtcTimingElement timingElement,
    ...
    ParsingLoadable<Long> parsingLoader = new ParsingLoadable<>(dataSource, Uri.parse(timingElement.value),
            C.DATA_TYPE_TIME_SYNCHRONIZATION, parser);
    parsingLoader.setVideoUrl(videoUrl);
    parsingLoader.setAudioUrl(audioUrl);
    startLoading(parsingLoader, new UtcTimestampCallback(), 1);
  }
  ...
  public DashMediaSource setDownloadedVideoUrl(String videoUrl) {
    this.videoUrl = videoUrl;
    return this;
  }

  public DashMediaSource setDownloadedAudioUrl(String audioUrl) {
