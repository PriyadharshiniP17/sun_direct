  ...
  protected Pair<Period, Long> parsePeriod(XmlPullParser xpp, String baseUrl, long defaultStartMs)
    ...
    do {
      ...
      if (XmlPullParserUtil.isStartTag(xpp, "BaseURL")) {
        ...
        AdaptationSet adaptationSet = parseAdaptationSet(xpp, baseUrl, segmentBase);
        if (adaptationSet != null
                && adaptationSet.representations != null
  ...
  protected AdaptationSet parseAdaptationSet(XmlPullParser xpp, String baseUrl,
    ...
    for (int i = 0; i < representationInfos.size(); i++) {
      if (TextUtils.isEmpty(videoUrl)
              && TextUtils.isEmpty(audioUrl)) {
        LoggerD.debugLog("Dash add representation "  + representationInfos.get(i) +" contentId- " + contentId );
        representations.add(buildRepresentation(representationInfos.get(i), contentId,
                drmSchemeDatas, inbandEventStreams));
        continue;
      }
      if (representationInfos.get(i).baseUrl.contains(audioUrl)
              || representationInfos.get(i).baseUrl.contains(videoUrl)) {
        LoggerD.debugLog("Dash adding only representation " + representationInfos.get(i) + " contentId- " + contentId + " for downloads");
        representations.add(buildRepresentation(representationInfos.get(i), contentId,
                drmSchemeDatas, inbandEventStreams));
      }
