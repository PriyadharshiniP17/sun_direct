...
class DraggableViewCallback extends ViewDragHelper.Callback {
  ...
  private static final int MINIMUM_DX_FOR_HORIZONTAL_DRAG = 25;
  private static final int MINIMUM_DY_FOR_VERTICAL_DRAG = 20;
...
//  private static final int MIN_DY = 20;
  ...
  private ViewDragHelper.Callback mDraggableCallbackListener;
  ...
  public void onViewDragStateChanged(int state) {
    ...
    notifyOnViewDragStateChanged(state);
    mDraggingState = state;
  }

  private void onStartDragging() {
    notifyOnStartDragging();
  }

  private void notifyOnStartDragging() {
    if (mDraggableCallbackListener != null) {
      mDraggableCallbackListener.onViewDragStateChanged(DraggablePanel.ON_DRAG_START);
    }
  }

  private void onStopDraggingToClosed() {
    notifyOnStopDragging();
  }

  private void notifyOnStopDragging() {
    if (mDraggableCallbackListener != null) {
      mDraggableCallbackListener.onViewDragStateChanged(DraggablePanel.ON_DRAG_STOPPED);
    }
  }

  private void notifyOnViewDragStateChanged(int state) {
    if (mDraggableCallbackListener != null) {
      mDraggableCallbackListener.onViewDragStateChanged(state);
    }
  }
  ...
  public void setDraggableViewCallbackListener(ViewDragHelper.Callback draggableCallbackListener) {
    this.mDraggableCallbackListener = draggableCallbackListener;
  }

  private void notifyOnViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
    if (mDraggableCallbackListener != null) {
      mDraggableCallbackListener.onViewPositionChanged(changedView, left, top, dx, dy);
    }
  }
