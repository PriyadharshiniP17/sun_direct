...
public abstract class Transformer {
  ...
  public void setViewWidth(int topFragmentWidth){
    if (topFragmentWidth > 0) {
      Log.d("mini player","setViewWidth: " + topFragmentWidth);
      originalWidth = topFragmentWidth;
      RelativeLayout.LayoutParams layoutParams =
              (RelativeLayout.LayoutParams) view.getLayoutParams();
      layoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT;
      view.setLayoutParams(layoutParams);
      LoggerD.debugHooqVstbLog("dragView.getWidth()- " + view.getWidth() +
  ...
  public int getOriginalWidth() {
    LoggerD.deebugFullScreen("view.getMeasuredWidth()- " + view.getMeasuredWidth());
    if (originalWidth == 0) {
  ...
  public boolean isDraggedMinimumRange() {
    int parentHeight = parent.getHeight();
    float viewYPosition = ViewCompat.getY(view) + (view.getHeight() * 0.25f);
    return viewYPosition > (parentHeight * 0.25);
  }
}
